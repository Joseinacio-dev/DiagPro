...
test('preload carrega no sandbox e delega a criação do scanId ao processo principal', async () => {
  ...
  assert.equal(typeof exposed.diagpro?.readSavedSession, 'function')
  assert.equal(typeof exposed.diagpro?.saveSession, 'function')
  assert.equal(typeof exposed.diagpro?.clearSavedSession, 'function')
